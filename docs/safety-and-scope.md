...
このドキュメントは、本プロジェクトが**読み取り専用(READ-ONLY)**であることの設計原則と、それを裏付ける具体的なコード上の根拠をまとめたものです。技術的なデータフローの詳細は [`architecture.md`](architecture.md)、機能一覧は [`README.md`](../README.md)、Control Plane境界の詳細は [`control-plane-boundary.md`](control-plane-boundary.md) を参照してください。
...
アーキテクチャは3つのplaneに分かれています(全体像は [`control-plane-boundary.md`](control-plane-boundary.md) を参照)。

- **Data Plane** — 公開データの取得と表示(BFF経由のCelesTrak/SatNOGS、SGP4、パス予測、NETウィンドウ、Simulator、Replay、advisory、ops checklist)。実際の外部I/Oを持つ唯一のplaneであり、常に一方向(GETのみ)。
- **Rehearsal Plane** — LIVE_READ_ONLY / REPLAYのコマンドコンソール。ローカル完結・ウォールクロック駆動・インメモリの訓練ログで、I/Oはゼロ。
- **Control Plane** — `src/services/control/`。v0.2.0ではインターフェースと唯一の永続的disabledアダプタのみで、実際のcapabilityは実装されておらず、どのコードパスからも到達できません。
...
### Control Plane: インターフェースのみ、永続的にdisabled

`src/services/control/ControlPlane.ts` は `ControlPlanePort` インターフェースと `ControlPlaneCapabilities` の形状を定義しており、5つのcapabilityフラグ(`canTransmitCommand`, `canTransmitRf`, `canControlAntenna`, `canControlGroundStation`, `canPerformOtaUpdate`)はすべて `boolean` ではなく**リテラル型 `false`** です — これを `true` に広げるには型宣言自体を書き換える必要があり、これは目に見える・レビュー可能な差分であって、実行時のトグルではありません。`ControlPlaneStatus` はメンバーを1つ(`"DISABLED"`)しか持ちません。唯一の実装 `DisabledControlPlaneAdapter`(`src/services/control/DisabledControlPlane.ts`)はコンストラクタ引数を一切取らず(送信先・デバイス・秘密情報といった設定対象自体が存在しない)、5つの制御メソッドはそれぞれ厳密に `throw new Error(...)` するだけです。

任意のビルド時フラグ `VITE_CONTROL_PLANE_MODE` は `parseControlPlaneMode()` によって読まれ、`"disabled"`(大文字小文字・空白は無視)と未設定のみが静かに受理されます。それ以外の値もすべて `DISABLED` に解決されます — `DisabledControlPlaneAdapter` 以外のアダプタを構築するコードパスはどこにも存在しません — が、設定ミスがサイレントに握りつぶされることのないよう、ストアが `WARN` `CTRL` イベントをログに記録します。将来*実際の*Control Planeを検討する場合に必要となる要件の全リストを含む詳細は [`control-plane-boundary.md`](control-plane-boundary.md) にまとめています。
...
- **Control Planeステータスチップ** — `TopBar.tsx` は `ControlPlaneStatusChip` も常時レンダリングし、`CONTROL PLANE: DISABLED` と(LIVE_READ_ONLY/REPLAYでは)`REHEARSAL PLANE: LOCAL ONLY — NOT TRANSMITTED` を表示専用で示します。ボタン・クリックハンドラ・設定用UIは一切持ちません。
...
- **Ops checklistとadvisoryはロード中に自信過剰な表示をしない** — 両者とも同一の `OperationalSnapshot`(`src/domain/operationalAssessment.ts`)から導出され、`ProviderRequestState` でゲートされています: リクエストが `NOT_REQUESTED` または `LOADING` の間、チェックリストは `PENDING`/`CHECKING` を表示し、時期尚早な `FAIL` の代わりにadvisoryは何も発火しません。SatNOGSトークン未設定は(障害を表す)`FAIL` ではなく(運用者へのアクション項目を表す)`CONFIG_REQUIRED` として表面化します。ゲーティングルールの全体は [`architecture.md`](architecture.md) を参照してください。
...
関連ドキュメント: [`README.md`](../README.md) ・ [`architecture.md`](architecture.md) ・ [`control-plane-boundary.md`](control-plane-boundary.md)
